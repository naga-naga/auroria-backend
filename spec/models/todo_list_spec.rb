...
RSpec.describe TodoList do
  describe 'todo_items の関連付けについて' do
    it 'TodoList から辿れる' do
      ...
      expect(TodoList.find(todo_list.id).todo_items).to contain_exactly(TodoItem.find(todo_item.id))
    end
  ...
  describe 'title について' do
    it '空文字列でない文字列の場合 valid' do
      todo_list = TodoList.build(title: 'foo')
      expect(todo_list).to be_valid
    end

    it '空文字列の場合 valid' do
      todo_list = TodoList.build(title: '')
      expect(todo_list).to be_valid
    end

    it 'nil の場合 invalid' do
      todo_list = TodoList.build(title: nil)
